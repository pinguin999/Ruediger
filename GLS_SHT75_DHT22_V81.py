...
GPIO.setwarnings(False) # tells Python not to print GPIO warning messages to the screen 
...
GPIO.setup(12, GPIO.OUT)  ## Setup GPIO Pin 12 (GPIO18) to OUT; Taste 2 (Pin4) FS20 Ein
GPIO.output(12, True)   # setzt den Pin12 auf High = 3,3V
GPIO.setup(13, GPIO.OUT)  ## Setup GPIO Pin 13 (GPIO27) to OUT; Taste 1 (Pin2) FS20 Aus
...
while True:
    while True:
        while True:
            ...
            try:
                ...
                rFo = sht2.read_rh()
                To = sht2.read_t()
            except (ShtCommandFailure,ShtCRCCheckError) as err:
    ...
    print (time.strftime('%d/%m/%y %H:%M'))  #dd/mm/yyyy hh:mm
    print 'Ti: {0:>+5.1f}*C rFi: {1:>2.1f}% aFi: {2:>4.1f}g/m3' .format(Ti,rFi,aFi)
    print 'To: {0:>+5.1f}*C rFo: {1:>2.1f}% aFo: {2:>4.1f}g/m3' .format(To,rFo,aFo)
    ...
    lcd.lcd_display_string("{0:>5.1f} {1:>3.0f}% {2:>5.1f}g/m3".format(Ti,rFi,aFi), 3) 
    lcd.lcd_display_string("{0:>5.1f} {1:>3.0f}% {2:>5.1f}g/m3".format(To,rFo,aFo), 4)
    ...
    if (aFo < aFi) and (rFi > ukrFi) and (To > mTo) or (Ti > To) and (Ti > mxTi) and (rFi < ukrFi):
        ...
        while min > -1:
            while sec > 0:
                ...
                min1 = ('%02.f' % min)
                lcd.lcd_display_string("{0} {1:>2}:{2:1}".format(Empfehlung,str(min1),str(sec1)), 1)  
                #sys.stdout.write('\r'),
...
#        time.sleep(0.01)
    ...
    while min > -1:
        while sec > 0:
            ...
            min2 = ('%02.f' % min)
            lcd.lcd_display_string("{0} {1:>2}:{2:1}".format(Wartezeit,str(min2),str(sec2)), 1)  
            #sys.stdout.write('\r'),
            #sys.stdout.write(str(min1) + c + str(sec1)),     #Var1K: Zeile funktioniert
            #sys.stdout.flush()                             #Var1K: Zeile2 funktioniert            
